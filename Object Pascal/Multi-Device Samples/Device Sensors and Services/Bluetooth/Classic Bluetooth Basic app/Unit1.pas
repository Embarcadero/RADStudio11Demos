...
uses
  ...
  FMX.ImgList, FMX.ListView.Adapters.Base, FMX.Memo.Types;
...
begin
  if ManagerConnected then
    if (ComboboxDevices.ItemIndex > -1) and (ComboboxDevices.ItemIndex < FDiscoverDevices.Count) then
...
begin
  if ManagerConnected then
    if (ComboboxPaired.ItemIndex > -1) and (ComboboxPaired.ItemIndex < FPairedDevices.Count) then
